...
| `HTTP오류` | 키 만료·무효, 모델명 변경, rate limit (첫 오류 메시지에 응답 본문이 같이 찍힌다) |
...
4. 카테고리당 최대 30건을 **8건씩 끊어, 4개씩 병렬로** LLM 호출해 번역+재구성+요약 (덩어리 단위로 실패 격리 — 한 덩어리가 죽어도 나머지는 살아남음)
...
>
> 병렬로 부르는 이유: 끊어 부르면 호출이 11건 → 35건으로 늘어 순차 실행 시 30분 워크플로 제한을 넘긴다(실제로 넘겨서 실행이 취소됐다). 청크끼리는 의존이 없어 4개씩 동시에 부르고, 결과는 원래 기사 순서대로 다시 이어 붙인다.
...
| LLM 호출 총합 | 실행 전체 900초 | `llm_client.LLM_TIME_BUDGET_SECONDS` |
| 주식 시세 조회 | 90초 | `stock_data` |
| 워크플로 전체 | 45분 | `daily_briefing.yml` |
