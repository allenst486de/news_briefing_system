...
| LLM 호출 총합 | 실행 전체 1800초 | `llm_client.LLM_TIME_BUDGET_SECONDS` |
| 원문 본문 수집 | 실행 전체 300초 | `article_body._TOTAL_BUDGET_SECONDS` |
| 주식 시세 조회 | 90초 | `stock_data` |
| 워크플로 전체 | 45분 | `daily_briefing.yml` |

이 값들은 서로 맞물려 있어 하나만 바꾸면 조용히 품질이 떨어진다. 실측 기준선:

- 청크 1건(기사 8건) → 한국어 출력 약 3,500토큰 → **호출당 약 87초**
- 총 35호출 ÷ 워커 6 = 6라운드 → **약 520초** (LLM 예산의 29%)
- 전체 실행 약 10분
