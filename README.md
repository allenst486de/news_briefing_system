...
겉으로는 '전부 실패'로만 보이기 때문에, 실행 시작 시 키마다 최소 호출(출력 8토큰)로 찔러 보고
결과를 남긴다. 쓸 수 없는 키가 있으면 그 카테고리는 **살아있는 다른 키로 자동 대체**되고,
동시 호출 수도 살아있는 키 개수에 맞춰 조정된다(키 1개면 3, 8개면 16).

```
**API 키 점검**
politics (공용)  정상
world            HTTP 401: {"status":401,"title":"Unauthorized",...}
```
