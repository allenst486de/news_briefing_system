...
매일 오전 6시(KST)에 국내외 25개 언론사에서 8개 분야 뉴스를 수집해 LLM으로 번역·재구성·요약하고, 포털 스타일 웹사이트와 텔레그램으로 전달하는 완전 자동화 시스템입니다.

## 📋 주요 기능

- **8개 분야 자동 수집**: 🏛️ 정치 · 💰 경제 · 👥 사회 · 🌱 생활 · 🎭 문화 · 💻 IT · 🔬 과학 · 🌍 국제
- **25개 언론사**: 국내 — 연합뉴스, 연합뉴스TV, 경향신문, 한겨레, 동아일보, 한국경제, 아시아경제, 연합인포맥스, 전자신문, ZDNet Korea, 노컷뉴스, SBS, 오마이뉴스, 프레시안, 구글 뉴스 / 해외 — BBC, NYT, The Guardian, Al Jazeera, Financial Times, Nature, ScienceDaily, WIRED, TechCrunch, Ars Technica. 전체 목록은 `src/collectors/sources.py` 참고
- **국내/해외 분리**: 카테고리마다 국내 20건 + 해외 20건을 따로 수집해 페이지에서 탭으로 전환한다(합쳐서 30건만 자르면 해외 기사가 국내에 밀려 거의 안 보였다)
- **오래된 기사 차단**: 발행일이 3일보다 오래된 기사는 버린다. 피드가 살아 있어도 갱신을 멈춘 곳이 있어 3년 전 기사가 그대로 올라왔다(실측: CNN 1217일, WSJ 562일, 경향 과학 513일 — 셋 다 목록에서 제거)
