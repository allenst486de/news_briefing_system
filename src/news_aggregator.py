...
STANDARD_CATEGORIES = {
    ...
    "world_general": ["world"],
    "world_economy_politics": ["business", "politics"]
}
...
class NewsAggregator:
    ...
    def _map_category(self, source: str, std_category: str) -> str:
        """
        표준 카테고리를 source별 카테고리로 변환
        """
        reverse_map = {
            v: k for k, v in SOURCE_CATEGORY_MAP[source].items()
        }
        return reverse_map.get(std_category)
    ...
    def collect_all_news(self) -> Dict[str, List[NewsArticle]]:
        ...
        for source, collector in collectors.items():
            ...
            for source_category, std_category in SOURCE_CATEGORY_MAP[source].items():
                if std_category not in sum(STANDARD_CATEGORIES.values(), []):
                    continue
